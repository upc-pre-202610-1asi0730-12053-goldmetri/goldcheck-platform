using GoldMetrics.GoldCheck.Platform.MonitoringTelemetry.Application.CommandServices;
        ...
        : ITemperatureCommandService
    {
        ...
        public async Task<Result<TemperatureReading>> Handle(AnalyseEngineOilTemperatureCommand command, CancellationToken cancellationToken = default)
        {
            var reading = await FindLatestByAsset(command.AssetId, cancellationToken);
            if (reading is null)
                return Result<TemperatureReading>.Failure(MonitoringTelemetryError.TemperatureReadingNotFound, localizer[nameof(MonitoringTelemetryError.TemperatureReadingNotFound)]);
            try
            {
                reading.AnalyseOilTemperature(command);
                await unitOfWork.CompleteAsync(cancellationToken);
                return Result<TemperatureReading>.Success(reading);
            }
            catch (ArgumentException)
        ...
        public async Task<Result<TemperatureReading>> Handle(AnalyseEngineFuelTemperatureCommand command, CancellationToken cancellationToken = default)
        {
            var reading = await FindLatestByAsset(command.AssetId, cancellationToken);
            if (reading is null)
                return Result<TemperatureReading>.Failure(MonitoringTelemetryError.TemperatureReadingNotFound, localizer[nameof(MonitoringTelemetryError.TemperatureReadingNotFound)]);
            try
            {
                reading.AnalyseFuelTemperature(command);
                await unitOfWork.CompleteAsync(cancellationToken);
                return Result<TemperatureReading>.Success(reading);
            }
            catch (ArgumentException)
