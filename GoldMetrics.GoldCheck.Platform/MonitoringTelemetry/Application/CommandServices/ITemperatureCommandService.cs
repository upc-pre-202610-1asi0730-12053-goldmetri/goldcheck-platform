...
public interface ITemperatureCommandService
{
    ...
    Task<Result<TemperatureReading>> Handle(AnalyseEngineFuelTemperatureCommand command, CancellationToken cancellationToken = default);
    Task<Result<TemperatureReading>> Handle(DetectTemperatureAnomalyCommand command, CancellationToken cancellationToken = default);
}