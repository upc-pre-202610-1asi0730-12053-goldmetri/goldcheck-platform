    ...
    IStringLocalizer<SubscriptionsBillingMessages> localizer) : ISubscriptionsBillingCommandService
{
    ...
    public async Task<Result<UserSubscription>> CheckUserPlanAsync(CheckUserPlanCommand command, CancellationToken ct = default)
    {
        try
        {
            var subscription = await repository.FindByUserIdAsync(command.UserId, ct);
            if (subscription is null) return NotFound<UserSubscription>();
            subscription.CheckUserPlan(command);
            repository.Update(subscription);
            await unitOfWork.CompleteAsync(ct);
            return Result<UserSubscription>.Success(subscription);
        }
    ...
    public async Task<Result<UserSubscription>> AssignFeaturesAsync(AssignFeaturesCommand command, CancellationToken ct = default)
    {
        try
        {
            var subscription = await repository.FindByUserIdAsync(command.UserId, ct);
            if (subscription is null) return NotFound<UserSubscription>();
            subscription.AssignFeatures(command);
            repository.Update(subscription);
            await unitOfWork.CompleteAsync(ct);
            return Result<UserSubscription>.Success(subscription);
        }
    ...
    public async Task<Result<UserSubscription>> RequestAccessAsync(RequestAccessCommand command, CancellationToken ct = default)
    {
        try
        {
            var subscription = await repository.FindByUserIdAsync(command.UserId, ct);
            if (subscription is null) return NotFound<UserSubscription>();
            subscription.RequestAccess(command);
            repository.Update(subscription);
            await unitOfWork.CompleteAsync(ct);
            return Result<UserSubscription>.Success(subscription);
        }
