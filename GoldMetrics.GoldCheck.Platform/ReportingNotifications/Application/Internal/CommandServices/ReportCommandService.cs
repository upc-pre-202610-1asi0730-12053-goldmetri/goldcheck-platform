    ...
    : IReportCommandService
{
    ...
    public async Task<Result<Report>> Handle(DownloadReportCommand command, CancellationToken cancellationToken)
    {
        var report = await FindReport(command.Id, cancellationToken);
        if (report is null) return NotFound();
        try
        {
            report.DownloadReport(command);
            reportRepository.Update(report);
            await unitOfWork.CompleteAsync(cancellationToken);
            return Result<Report>.Success(report);
        }
